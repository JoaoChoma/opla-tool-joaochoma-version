...
public class ExecutaIndicadores2Objetives {
	...
	public static void main(String[] args) throws Exception {
		...
		String[] plas = { "bet" };
		String caminho;
		caminho = "/home/note/analise-experimento-genetico-original-3objetivos/analise-quantitativa-30bj/AnaliseQualitativaBET/";
		...
		for (String software : plas) {
			...
			for (String algorithm : algoritmos) {
				...
				problem.setnumberOfObjectives(3);
...
//				JOptionPane.showMessageDialog(null, "true");
				...
				// pareto real
				double[] min = mu.getMinimumValues(ss.writeObjectivesToMatrix(), 3); 
				...
				for (int i = 0; i < melhoresSolucoesAlgoritmo.length; i++) {
					...
					for (int run = 1; run < 31; run++) {
						try {
							...
							value = indicators.getIGD(front,ss,3);
							...
							value = indicators.getGD(front,ss,3);
